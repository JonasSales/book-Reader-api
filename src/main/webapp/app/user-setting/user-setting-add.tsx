...
export default function UserSettingAdd() {
  ...
  useDocumentTitle(t('userSetting.add.headline'));
  ...
  const createUserSetting = async (data: UserSettingDTO) => {
    window.scrollTo(0, 0);
    try {
      await axios.post('/api/userSettings', data);
      navigate('/userSettings', {
            state: {
              msgSuccess: t('userSetting.create.success')
            }
          });
    } catch (error: any) {
  ...
  return (<>
    <div className="flex flex-wrap mb-6">
      <h1 className="grow text-3xl md:text-4xl font-medium mb-2">{t('userSetting.add.headline')}</h1>
      <div>
        <Link to="/userSettings" className="inline-block text-white bg-gray-500 hover:bg-gray-600 focus:ring-gray-200 focus:ring-4 rounded px-5 py-2">{t('userSetting.add.back')}</Link>
      </div>
    </div>
    <form onSubmit={useFormResult.handleSubmit(createUserSetting)} noValidate>
      ...
      <input type="submit" value={t('userSetting.add.headline')} className="inline-block text-white bg-blue-600 hover:bg-blue-700 focus:ring-blue-300  focus:ring-4 rounded px-5 py-2 cursor-pointer mt-6" />
    </form>
  </>);
}
